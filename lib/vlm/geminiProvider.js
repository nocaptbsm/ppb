...
${domString}`;
      ...
      console.warn('[GeminiProvider] Error calling VLM, falling back to Smart Mock Mode:', error);
      const simulated = await this._simulateResponse(minimumContext, userCommand);
      return {
        ...simulated,
        reasoning: `[VLM Quota Fallback] ${simulated.reasoning} (API Error: ${error.message})`,
        latencyMs: Math.round(performance.now() - start),
      };
    }
  }
  ...
  async _simulateResponse(minimumContext, userCommand) {
    ...
    if (command.includes('open') || command.includes('go to') || command.includes('navigate')) {
      ...
      else if (command.includes('wikipedia')) url = 'https://en.wikipedia.org';
      else if (command.includes('amazon')) url = 'https://www.amazon.in';
      else if (command.includes('sih') || command.includes('gov')) url = 'https://sih.gov.in';
      
      action = { type: 'navigate', url, confidence: 0.95 };
      reasoning = `Navigating active browser window to URL: ${url}`;
    }
    
    else if (command.includes('click') || command.includes('select') || command.includes('choose')) {
