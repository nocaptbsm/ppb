...
class DirectDOMExecutor extends ActionExecutor {
  ...
  _findElement(selector, root) {
    ...
    if (!el && selector === '#search-input') {
      // Fallback search input selectors for external websites (Google, Amazon, etc.)
      el = root.querySelector('input[name="q"]') || 
           root.querySelector('input[type="search"]') || 
           root.querySelector('input[placeholder*="search" i]') ||
           root.querySelector('input[type="text"]');
    }
    if (!el && selector === '#search-btn') {
      // Fallback search buttons
      el = root.querySelector('button[type="submit"]') ||
           root.querySelector('input[type="submit"]') ||
           root.querySelector('.search-button') ||
